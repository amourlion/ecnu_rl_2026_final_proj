# Exp4 Vanilla DQN - Requester Reward

This experiment uses the shared DQN training skeleton with the standard DQN
network and uniform replay. It optimizes the requester-side reward, so it is the
main experiment for answering how reinforcement learning can maximize requester
benefit.

Run on Apple Silicon with uv:

```bash
cd exps/dqn_requester_reward
uv venv .venv
